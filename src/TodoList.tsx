...
const createTodoWithUpdatedStatus = ({
  updatedStatus,
  item,
}: {
...
const isItem = (item: ITodoItem, searchItem: ITodoItem) =>
...
export default function TodoList() {
  ...
  const deleteTodo = (deleted: ITodoItem) => {
    setTodos((current: ITodoItem[]) =>
      current.filter((item) => !isItem(item, deleted)),
    );
  };
    ...
    setTodos((current) => {
      return current.map((item: ITodoItem) => {
        if (isItem(item, updated)) {
          return createTodoWithUpdatedStatus({
            item,
